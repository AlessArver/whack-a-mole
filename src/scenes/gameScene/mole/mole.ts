...
let ifElseChoiceHole = (array, arrayName, moles, mole, moleXFirst, moleXSecond) => {
    if (mole.x === moleXFirst && !array.length || mole.x === moleXSecond && !array.length) addMole(array, arrayName, moles, mole)
}
...
const choiceCreateMole = (resolve) => {
    ...
    moles.width = 388;
    moles.y = (window.app.view.height - moles.height) - 50

    switch (selectMole) {
        case "simpleMole": {
            createMole(resolve, moles, moleXSimple[Math.floor(Math.random() * moleXSimple.length)], 0, 0, 60, 150, simpleMoleOnClick);
            break;
        }
        case "strongMole": {
            createMole(resolve, moles, moleXStrong[Math.floor(Math.random() * moleXStrong.length)], 60, 0, 70, 150, strongMoleOnClick);
            break;
        }
    }
};
